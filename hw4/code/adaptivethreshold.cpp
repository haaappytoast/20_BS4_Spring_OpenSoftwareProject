...
int main() {
	...
	cvtColor(input, input_gray, CV_BGR2GRAY); // Converting image to gray
	...
	output = adaptive_thres(input_gray, 2, 0.8); //Fix with uniform mean filtering with zero paddle
...
Mat adaptive_thres(const Mat input, int n, float bnumber) {
	...
	// Initialiazing Kernel Matrix : using mean intensity
	...
	for (int i = 0; i < row; i++) { //for each pixel in the output
		for (int j = 0; j < col; j++) {
			...
			//makes output image's pixel intensity to 255 if the intensity of the input image is bigger
