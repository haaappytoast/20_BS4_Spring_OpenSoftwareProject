...
#if (IM_TYPE == CV_8UC3)
typedef uchar G;	// G를 uchar 자료형의 별칭으로 정의한다. uchar = unsigned char [0~255]
typedef cv::Vec3b C;
...
float* cal_PDF(Mat& input) {
	...
	for (int i = 0; i < input.rows; i++)
		for (int j = 0; j < input.cols; j++)
			count[input.at<G>(i, j)]++; // [0~255] 정수들이 픽셀에 각각 얼만큼 있는지 센다.
