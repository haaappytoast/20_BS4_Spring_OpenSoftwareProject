...
Mat Gaussianfilter(const Mat input, int n, double sigma_t, double sigma_s) {
	...
	for (int i = n; i < row+n; i++) {
		for (int j = n; j < col+n; j++) {
			...
			output.at<double>(i - n, j - n) = sum1;		// b/c i, j started with index n index has to be subtracted
		}
	}
	return output;
}
...
Mat Laplacianfilter(const Mat input) {
	...
	for (int i = n; i < row + n; i++) {
		for (int j = n; j < col + n; j++) {
			...
			output.at<double>(i - n, j - n) = abs(sum1)*2000;
		}
	}
	return output;
}
...
Mat Mirroring(const Mat input, int n)
{
	...
	for (int i = n; i < row + n; i++) {
		for (int j = n; j < col + n; j++) {
			input2.at<double>(i, j) = input.at<double>(i - n, j - n);	//putting the original pixels into the input2 
		}
	}
	for (int i = n; i < row + n; i++) {		
