...
//	Mat input = imread("checkerboard2.jpg", CV_LOAD_IMAGE_COLOR);
	...
	//output = Mat::zeros(input_gray.size(), CV_32FC1);
		...
		for (int j = radius; j < col+radius; j++) {
			...
			for (int a = -radius; a <= radius; a++) {
				for (int b = -radius; b <= radius; b++) {
					...
					if (max < input_mirror.at<float>(i + a, j + b)) max = input_mirror.at<float>(i + a, j + b);
				}
			}
			if (max !=0 && max == input_mirror.at<float>(i, j)) corner_mat.at<uchar>(i - radius, j - radius) = 1;
			else corner_mat.at<uchar>(i - radius, j - radius) = 0;
		}
	}
	return input;
}
