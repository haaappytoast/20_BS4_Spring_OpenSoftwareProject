...
int main() {
	...
	input.convertTo(input, CV_64FC3, 1.0 / 255); 	// 8-bit unsigned char -> 64-bit floating point
...
Mat Gaussianfilter(const Mat input, int n, double sigma_t, double sigma_s) {
	...
	for (int i = n; i < row+n; i++) {
		for (int j = n; j < col+n; j++) {
			...
			for (int a = -n; a <= n; a++) {
				for (int b = -n; b <= n; b++) {
					sum_b += kernel.at<double>(a + n, b + n) * input_mirror.at<Vec3d>(i + a, j + b)[0];	//blue
					sum_g += kernel.at<double>(a + n, b + n) * input_mirror.at<Vec3d>(i + a, j + b)[1]; //green
					sum_r += kernel.at<double>(a + n, b + n) * input_mirror.at<Vec3d>(i + a, j + b)[2]; //red
				}
			}
			output.at<Vec3d>(i - n, j - n)[0] = sum_b;		// b/c i, j started with index n index has to be subtracted
...
Mat Laplacianfilter(const Mat input) {
	...
	for (int i = n; i < row + n; i++) {
		for (int j = n; j < col + n; j++) {
			...
			for (int a = -n; a <= n; a++) {
				for (int b = -n; b <= n; b++) {
					sum_b += kernel.at<double>(a + n, b + n) * input_mirror.at<Vec3d>(i + a, j + b)[0];	//blue channel
					sum_g += kernel.at<double>(a + n, b + n) * input_mirror.at<Vec3d>(i + a, j + b)[1];	//green channel
					sum_r += kernel.at<double>(a + n, b + n) * input_mirror.at<Vec3d>(i + a, j + b)[2];	//red channel
				}
			}	
			double temp = (sum_b + sum_g + sum_r) / 3.0;
			output.at<Vec3d>(i - n, j - n)[0] = abs(temp)*2000;
			output.at<Vec3d>(i - n, j - n)[1] = abs(temp) * 2000;
			output.at<Vec3d>(i - n, j - n)[2] = abs(temp) * 2000;
		}
	}
	return output;
}
...
Mat Mirroring(const Mat input, int n)
{
	...
	for (int i = n; i < row + n; i++) {
		for (int j = n; j < col + n; j++) {
			input2.at<Vec3d>(i, j)[0] = input.at<Vec3d>(i - n, j - n)[0];	//putting the original pixels into the input2 
	...
	for (int i = n; i < row + n; i++) {		
		for (int j = 0; j < n; j++) {		// left side of input2 of mirroring
		...
		for (int j = col + n; j < col2; j++) {		// right side of input2 of mirroring
	...
	for (int j = 0; j < col2; j++) {
		for (int i = 0; i < n; i++) {				// upper side of input2 of mirroring
		...
		for (int i = row + n; i < row2; i++) {		// down side of input2 of mirroring
