...
int main() {
	...
	output = unsharpmask(input, 1, 1, 1, "zero-paddle", 0.5); //Boundary process: zero-paddle, mirroring, adjustkernel
			...
			if (!strcmp(opt, "zero-paddle")) {
				...
				for (int a = -n; a <= n; a++) {
					for (int b = -n; b <= n; b++) {
						...
						if ((i + a <= row - 1) && (i + a >= 0) && (j + b <= col - 1) && (j + b >= 0)) { //if the pixel is not a border pixel
							sum1_b += kernel.at<float>(a + n, b + n) * (float)(input.at<C>(i + a, j + b)[0]);
							sum1_g += kernel.at<float>(a + n, b + n) * (float)(input.at<C>(i + a, j + b)[1]);
							sum1_r += kernel.at<float>(a + n, b + n) * (float)(input.at<C>(i + a, j + b)[2]);
						}
					}
				}
				...
				for (int a = -n; a <= n; a++) {
					for (int b = -n; b <= n; b++) {
						...
						sum1_b += kernel.at<float>(a + n, b + n) * (float)(input.at<C>(tempa, tempb)[0]);
						sum1_g += kernel.at<float>(a + n, b + n) * (float)(input.at<C>(tempa, tempb)[1]);
						sum1_r += kernel.at<float>(a + n, b + n) * (float)(input.at<C>(tempa, tempb)[2]);
					}
				}
				output.at<C>(i, j)[0] = (G)sum1_b;
				output.at<C>(i, j)[1] = (G)sum1_g;
